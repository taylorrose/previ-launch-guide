...
// If needed, import employerData to validate codes, but optional
// import { employerData } from '@/data/employerData'

/**
 * Extract first path segment if you want a code-based route,
 * e.g. "/abc123/introduction" => "abc123"
 */
function getCodeSegment(pathname) {
...
function useAutocomplete({ close }) {
    ...
    // ADDED: read the current route to detect code
    let pathname = usePathname()
    let codeSegment = getCodeSegment(pathname)

    function navigate({ itemUrl }) {
        if (!itemUrl) return

        // ADDED: If itemUrl includes /[code], replace it with the actual code
        // E.g., "/[code]/overview" => "/abc123/overview"
        if (codeSegment && itemUrl.includes('/[code]')) {
            itemUrl = itemUrl.replace('/[code]', `/${codeSegment}`)
        }
        // Otherwise, if itemUrl is a bare route (like "/quickstart"), prefix
        else if (codeSegment && itemUrl.startsWith('/')) {
            itemUrl = `/${codeSegment}${itemUrl}`
        }
    ...
    return (
        <div className="group relative flex h-12">
            ...
            <input
                ...
                data-autofocus
                className={clsx(
                    'flex-auto appearance-none bg-transparent pl-10 text-zinc-900 outline-hidden placeholder:text-zinc-500 focus:w-full focus:flex-none sm:text-sm dark:text-white',
                ...
                onKeyDown={(event) => {
                    if (
                        ...
                        // In Safari, closing the dialog with the escape key can sometimes cause
                        // the scroll position to jump to the bottom of the page. This is a
                        // workaround for that until we can figure out a proper fix in Headless UI.
