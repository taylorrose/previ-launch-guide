...
export function Layout({ children, allSections }) {
    ...
    // e.g. "/" => []
    //      "/abc123" => ["abc123"]
    //      "/abc123/introduction" => ["abc123", "introduction"]
    const segments = pathname.split("/").filter(Boolean)

    // Hide only the Navigation if 0 segments ("/") or exactly 1 segment ("/abc123")
    const hideNav = segments.length <= 1

    // Condition for adding side margin
        ...
        <SectionProvider sections={allSections[pathname] ?? []}>
            <div className={containerClasses}>
                ...
                >
                    <div
                        className={
                            // If nav is hidden, remove the border and fixed width
                            hideNav
                    ...
                    >
                        ...
                        {/* Navigation is conditionally hidden */}
                        {!hideNav && (
